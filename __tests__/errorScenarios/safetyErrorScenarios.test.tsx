...
describe('Safety Error Scenarios', () => {
  ...
  describe('Validation Errors in Form', () => {
    ...
    it('handles non-numeric inputs', async () => {
      ...
      fireEvent.changeText(getByPlaceholderText('Longitude'), 'also not a number');
      fireEvent.changeText(getByPlaceholderText('Radius (m)'), 'definitely not a number');
      fireEvent.press(getByText('Add Safe Zone'));
    ...
    it('sanitizes suspicious or injection-like input', async () => {
      ...
      fireEvent.changeText(getByPlaceholderText('Latitude'), '"; DROP TABLE zones; --');
      fireEvent.changeText(getByPlaceholderText('Longitude'), '${process.exit()}');
      fireEvent.press(getByText('Add Safe Zone'));
  ...
  describe('Concurrent Access', () => {
    it('handles simultaneous zone event updates', async () => {
      ...
      const result = await Promise.all([
        safeZoneAlertManager.handleSafeZoneEvent('zone-1', 'enter', mockZone1, { latitude: 40.7128, longitude: -74.006 }),
        safeZoneAlertManager.handleSafeZoneEvent('zone-2', 'enter', mockZone2, { latitude: 40.7228, longitude: -74.016 }),
      ]);
  ...
  describe('State Corruption', () => {
    it('recovers from corrupted AsyncStorage data', async () => {
      (AsyncStorage.getItem as jest.Mock).mockResolvedValue('corrupted json data');
      await safeZoneAlertManager.initialize();
      const settings = safeZoneAlertManager.getSettings();
