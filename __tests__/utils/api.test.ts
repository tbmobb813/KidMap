...
const MOCK_COORDINATES = {
  origin: [40.7128, -74.0060] as [number, number], // New York City
...
describe('API Utils - Route Fetching', () => {
  ...
  describe('Route Structure Validation', () => {
    it('should return RouteResult with correct structure for walking mode', async () => {
      const result = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'walking');
    ...
    it('should return RouteResult with correct structure for bicycling mode', async () => {
      const result = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'bicycling');
    ...
    it('should return RouteResult with correct structure for transit mode', async () => {
      const result = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'transit');
      ...
      if (result) {
        ...
        const hasTransitStep = result.steps.some(step => step.travelMode === 'transit');
        const hasWalkingStep = result.steps.some(step => step.travelMode === 'walking');
    ...
    it('should validate coordinate bounds in route result', async () => {
      ...
      if (result?.bounds) {
        ...
        expect(result.bounds.northeast.lat).toBeGreaterThanOrEqual(result.bounds.southwest.lat);
        expect(result.bounds.northeast.lng).toBeGreaterThanOrEqual(result.bounds.southwest.lng);
      }
    });
  });
  ...
  describe('Travel Mode Specific Features', () => {
    ...
    it('should include transit line information for transit mode', async () => {
      ...
      if (result) {
        const transitSteps = result.steps.filter(step => step.travelMode === 'transit');
    ...
    it('should have appropriate durations for different modes', async () => {
      const walkingResult = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'walking');
      const bicyclingResult = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'bicycling');
      
      if (walkingResult && bicyclingResult) {
        // Bicycling should generally be faster than walking for the same route
        expect(bicyclingResult.totalDuration).toBeLessThanOrEqual(walkingResult.totalDuration);
      }
    });

    it('should handle driving mode if supported', async () => {
      try {
        const result = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'driving' as TravelMode);
  ...
  describe('Error Handling', () => {
    ...
    it('should handle API error statuses', async () => {
      ...
      const result = await fetchRoute(MOCK_COORDINATES.invalid, MOCK_COORDINATES.destination, 'walking');
      expect(result).toBeNull();
    });
  });
  ...
  describe('Input Validation', () => {
    it('should handle valid coordinate ranges', async () => {
      ...
      for (const coord of validCoords) {
        const result = await fetchRoute(coord, MOCK_COORDINATES.destination, 'walking');
    ...
    it('should handle all supported travel modes', async () => {
      ...
      for (const mode of supportedModes) {
        const result = await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, mode);
        if (result) {
  ...
  describe('Performance and Reliability', () => {
    ...
    it('should maintain consistency across identical requests', async () => {
      const request1 = fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'walking');
      const request2 = fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'walking');
    ...
    it('should complete requests within reasonable time', async () => {
      ...
      await fetchRoute(MOCK_COORDINATES.origin, MOCK_COORDINATES.destination, 'walking');
