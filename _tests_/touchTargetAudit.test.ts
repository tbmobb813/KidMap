import { auditTouchTarget, MIN_TOUCH_TARGET } from '@/utils/touchTargetAudit';
...
describe('touchTargetAudit', () => {
    ...
    it('warns for undersized components', () => {
        ...
        const result = auditTouchTarget({ name: 'TinyButton', width: 30, height: 30 });
    ...
    it('passes when hitSlop compensates for small visual size', () => {
        const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => { });
        const result = auditTouchTarget({ name: 'SlopButton', width: 36, height: 36, hitSlop: 8 }); // 36 + (8*2)=52
        expect(result).toBe(true);
