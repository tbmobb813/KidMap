...
const ParentDashboard: React.FC<ParentDashboardProps> = ({ onExit }) => {
  ...
  const pendingCheckIns = checkInRequests.filter(
  ...
  const handleRequestCheckIn = () => {
    ConfirmDialog.show({
      ...
      message: "Send a check-in request to your child?",
      confirmText: "Send Request",
      onConfirm: () => {
        requestCheckIn("Please check in and let me know you are safe", false);
        showToast("Your child will receive a check-in request", "success");
      },
    });
  };
  ...
  const handleDevicePing = (type: "location" | "ring") => {
    ...
    showToast(
      `${
        type === "location" ? "Location request" : "Device ping"
      } sent to child's device`,
      "success"
    );
  };

  const handleSendMessage = () => {
    // For now, keep Alert.prompt for text input - this is a system dialog for input
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: (message) => {
            if (message && message.trim()) {
              ...
              showToast("Your message has been sent to your child", "success");
            }
          },
        },
      ],
  ...
  const handleApproveCategory = (categoryId: string) => {
    ...
    showToast("The category has been approved and is now available", "success");
  };
          ...
          <View style={styles.locationCard}>
            ...
            <View style={styles.locationContent}>
              <Text style={styles.locationTitle}>
                {dashboardData.lastKnownLocation.placeName ||
                  "Unknown Location"}
              </Text>
            ...
            <View key={checkIn.id} style={styles.checkInCard}>
              ...
              <View style={styles.checkInContent}>
                ...
                <Text style={styles.checkInTime}>
                  {formatTime(checkIn.timestamp)} on{" "}
        ...
        {safeZones.length === 0 ? (
          <View style={styles.emptyStateCard}>
            ...
            <Text style={styles.emptyStateSubtitle}>
              Create safe zones to monitor when your child enters or leaves
              specific areas
            </Text>
            <Pressable
          ...
          safeZones.map((zone) => (
            <View key={zone.id} style={styles.safeZoneCard}>
              <Shield
                size={20}
                color={
                  zone.isActive ? Colors.success : theme.colors.textSecondary
                }
              />
        ...
        <TabButton
          ...
          icon={
            <Phone
              size={16}
              color={
                activeTab === "pings" ? "#FFFFFF" : theme.colors.textSecondary
              }
            />
          }
        />
