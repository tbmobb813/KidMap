...
const SafetyPanel: React.FC<SafetyPanelProps> = ({
  ...
  const { settings, addCheckInToDashboard, updateLastKnownLocation } =
    useParentalStore();
  ...
  const handleShareLocation = async () => {
    try {
      if (!currentLocation) {
        const locationError = handleLocationError({
      ...
      if (primaryContact && Platform.OS !== "web") {
        ...
        log.info("Location shared successfully", {
          contactId: primaryContact.id,
        });
  ...
  const handleSafeArrival = () => {
    ConfirmDialog.show({
      title: "I Made It!",
      message: "Let your family know you arrived safely?",
  ...
  const handlePhotoCheckIn = async () => {
    try {
      ...
      if (
        ...
        log.warn("Location warnings for photo check-in", {
      ...
      if (Platform.OS === "web") {
        ...
        input.onchange = async (e: any) => {
          ...
          try {
            ...
            log.info("Web photo check-in completed", {
              placeId: currentPlace.id,
            });
          } catch (error) {
      ...
      try {
        ...
        if (!permissionResult.granted) {
          const cameraError = handleCameraError({
      ...
      try {
        ...
        if (!result.canceled && result.assets[0]) {
          ...
          try {
            ...
            log.warn(
              "Failed to add check-in to parent dashboard",
              error as Error
            );
          ...
          const accuracyDescription = getLocationAccuracyDescription(
          ...
          <View style={styles.content}>
            ...
            <View style={styles.buttonRow}>
              ...
              <Pressable
                style={[
                  styles.safetyButton,
                  isPhotoLoading && styles.loadingButton,
                ]}
