...
const alertManagerFile = path.join(__dirname, '..', 'utils', 'safeZoneAlerts.ts');
...
const alertComponentFile = path.join(__dirname, '..', 'components', 'SafeZoneAlert.tsx');
...
const settingsFile = path.join(__dirname, '..', 'components', 'SafeZoneSettings.tsx');
...
if (fs.existsSync(geofencingFile)) {
  ...
  if (content.includes('safeZoneAlertManager') && content.includes('handleSafeZoneEvent')) {
...
const dashboardFile = path.join(__dirname, '..', 'components', 'ParentDashboard.tsx');
if (fs.existsSync(dashboardFile)) {
  const content = fs.readFileSync(dashboardFile, 'utf8');
  
  if (content.includes('SafeZoneSettings') && content.includes('SafeZoneAlertHistory')) {
