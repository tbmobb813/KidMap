...
export const createMockZone = (
  id: string,
  overrides: Partial<MockSafeZone> = {},
): MockSafeZone => ({
  id,
  name: `Test Zone ${id}`,
  latitude: 40.7128,
...
export const selectSafeZoneForEdit = (
  renderResult: RenderAPI,
  zoneName: string,
) => {
  const { getByText } = renderResult
...
export const testInvalidFormInputs = async (renderResult: RenderAPI) => {
  const scenarios = [
    ...
    },
    {
      ...
      errors: [
        ...
        'Latitude must be between -90 and 90',
        'Longitude must be between -180 and 180',
        'Radius cannot exceed 10000 meters',
      ],
    },
    {
      data: {
  ...
  for (const scenario of scenarios) {
    await fillFormAndExpectValidation(
      renderResult,
      scenario.data,
      scenario.errors,
    )
