...
import { createAsyncStorageMock } from './mockAsyncStorage'
...
export const createMockSafeZone = (
  overrides: Partial<(typeof mockSafeZones)[0]> = {},
) => ({
  id: `zone-${Date.now()}`,
  name: 'Custom Zone',
  radius: 150,
...
export const createMockEmergencyContact = (
  overrides: Partial<(typeof mockEmergencyContacts)[0]> = {},
) => ({
  id: `contact-${Date.now()}`,
  name: 'Custom Contact',
  phone: '+1122334455',
...
// Common test setup: mocks for native modules
import { createAsyncStorageMock } from './mockAsyncStorage'
