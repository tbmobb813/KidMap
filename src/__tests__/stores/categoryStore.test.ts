...
describe('Category Store', () => {
  ...
  describe('Category Selection', () => {
    it('should select and deselect categories', () => {
      const { setCategories, selectCategory, deselectCategory } =
    ...
    it('should clear all selected categories', () => {
      const { setCategories, selectCategory, clearSelectedCategories } =
        useCategoryStore.getState()
  ...
  describe('Custom Categories', () => {
    ...
    it('should remove custom categories', () => {
      ...
      const { addCustomCategory, removeCustomCategory } =
        useCategoryStore.getState()
  ...
  describe('Category Filtering', () => {
    it('should get places by category', () => {
      ...
      const { setCategories, getPlacesByCategory } = useCategoryStore.getState()
      ...
      if (schoolCategory) {
        const schoolPlaces = getPlacesByCategory?.(
          schoolCategory.id,
          mockPlaces,
        )
        expect(schoolPlaces?.length).toBeGreaterThan(0)

        const allSchools = schoolPlaces?.every(
          (place) => place.category === 'school',
        )
    ...
    it('should filter categories by selection', () => {
      const { setCategories, selectCategory, getSelectedCategories } =
        useCategoryStore.getState()
    ...
    it('should check if category is selected', () => {
      const { setCategories, selectCategory, isCategorySelected } =
        useCategoryStore.getState()
  ...
  describe('Category Search', () => {
    it('should search categories by name', () => {
      ...
      const hasSchoolCategory = schoolResults?.some((category) =>
        category.name.toLowerCase().includes('school'),
      )
  ...
  describe('Complete Category Flow', () => {
    ...
    it('should validate category system completeness', () => {
      ...
      const hasBasicCategories =
        state.categories.some((c) => c.name === 'School') &&
  ...
  describe('Error Handling', () => {
    ...
    it('should handle null/undefined operations gracefully', () => {
      const { addCategory, removeCategory, selectCategory } =
        useCategoryStore.getState()
  ...
  describe('State Persistence', () => {
    it('should maintain consistent state', () => {
      ...
      const { setCategories, setSelectedCategories, setCustomCategories } =
        useCategoryStore.getState()
