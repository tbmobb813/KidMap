...
import { fetchRoutes, FetchRoutesParams, getRouteServiceMetrics } from '@/services/routeService';
...
export function useRoutesQuery(origin: Place | null, destination: Place | null, mode: TravelMode, options: RouteOptions) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (cacheHit) mark(`routes_cache_hit:${keyId}`); else mark(`routes_cache_miss:${keyId}`);
            mark(`routes_fetch_end:${keyId}`);
            measure(`routes_fetch_duration:${keyId}`, `routes_fetch_start:${keyId}`, `routes_fetch_end:${keyId}`);
            track({ type: 'route_fetch', mode, durationMs: Date.now() - startTs, cacheHit });
