  ...
  persist(
    (set, get) => ({
      ...
      completeTrip: (tripData: TripData) =>
        set((state) => {
          const updatedStats = achievementEngine.updateUserStats(
            state.userStats,
            tripData,
          )
          ...
          const achievementPoints =
            achievementEngine.calculateTotalPoints(newAchievements)
          const finalStats = {
            ...updatedStats,
            totalPoints: updatedStats.totalPoints + achievementPoints,
            level:
              Math.floor((updatedStats.totalPoints + achievementPoints) / 200) +
              1,
          }
