...
export type TelemetryEvent =
  ...
  | {
      ...
      action: "quiz" | "more" | "story_generated";
...
class ConsoleAdapter implements TelemetryAdapter {
  record(e: TelemetryEvent) {
    if (process.env.NODE_ENV !== "test") console.log("[telemetry]", e);
  }
  flush() {
    /* noop */
  }
...
// Global telemetry settings with privacy controls
let adapter: TelemetryAdapter = new ConsoleAdapter();
let telemetryEnabled = true;

export function setTelemetryAdapter(a: TelemetryAdapter) {
  adapter = a;
}
...
export function track<E extends Omit<TelemetryEvent, "ts">>(e: E) {
  if (!telemetryEnabled) return; // Privacy control
...
export function getMemoryEvents(): TelemetryEvent[] {
  return adapter instanceof MemoryAdapter
    ? [...(adapter as MemoryAdapter).events]
    : [];
}
