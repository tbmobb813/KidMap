...
class TransitAPI {
    ...
    try {
      ...
      return [
        ...
        },
        {
          ...
          color: '#00933c',
          status: 'delayed',
          message: 'Delays up to 10 minutes',
        },
        {
          id: 'l',
          name: 'L',
        ...
        },
        {
          ...
          color: '#fccc0a',
          status: 'alert',
          message: 'Weekend service changes',
        },
        {
          id: 'q',
          name: 'Q',
  ...
  private getNYCStations(
    ...
    const stations: TransitStation[] = [
      ...
      },
      {
        ...
        location: { lat: 40.752769, lng: -73.97736 },
        routes: ['4', '5', '6', '7', 'S'],
        arrivals: [],
      },
      {
  ...
  private getMockArrivals(stationId: string): TransitArrival[] {
    ...
    routes.forEach((route) => {
      ...
      for (
        let i = 0;
        i < ARRIVALS_MIN + Math.floor(Math.random() * ARRIVALS_VARIATION);
        i++
      ) {
        const minutes =
          MINUTES_BASE +
          i * MINUTES_STEP +
          Math.floor(Math.random() * MINUTES_VARIATION)
        arrivals.push({
          route,
          ...
          delay:
            Math.random() > 0.8
              ? Math.floor(Math.random() * DELAY_VARIATION)
              : undefined,
        })
      }
    })
...
export async function getTransitDirections(
  ...
  try {
    ...
    const distance = transitAPI['calculateDistance'](
    ...
    return {
      ...
      accessibilityNotes: [
        'Hold handrails',
        'Stay with adult if traveling with children',
      ],
    }
  } catch (error) {
