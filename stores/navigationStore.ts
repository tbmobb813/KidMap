...
import { invariant } from "@/utils/error/invariant";
import { verifyLocationProximity } from "@/utils/location/locationUtils";
import { loadPersistedState, savePersistedState } from "@/utils/persistence/persistence";
...
export const useNavigationStore = create<NavigationState>((set, get) => ({
    ...
    try {
      ...
      if (data) {
        set({
          ...
          accessibilitySettings:
            data.accessibilitySettings ?? get().accessibilitySettings,
          photoCheckIns: data.photoCheckIns ?? [],
          selectedTravelMode: (
            ["transit", "walking", "biking", "driving"] as const
          ).includes((data as any).selectedTravelMode)
            ? ((data as any).selectedTravelMode as TravelMode)
            : "transit",
  ...
  addToFavorites: (place) =>
    set((state) => {
      ...
      const next = {
        favorites: [...state.favorites, updatedPlace],
  ...
  findRoutesAsync: async () => {
    ...
    try {
      ...
      invariant(
        origin.name.length > 0 && destination.name.length > 0,
        "Origin or destination missing name"
      );
  ...
  addPhotoCheckIn: (checkIn) => {
    ...
    set((state) => {
      const next = {
        photoCheckIns: [
          ...state.photoCheckIns,
          { ...parsed.data, id: Date.now().toString() },
        ],
      } as Partial<NavigationState>;
      schedulePersist(() => ({ ...state, ...next } as NavigationState));
      return next;
