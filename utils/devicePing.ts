...
class DevicePingManager {
  ...
  async sendPingToChild(
    type: PingRequest['type'],
    message?: string,
    urgency: PingRequest['urgency'] = 'medium',
  ): Promise<string> {
    const pingId = Date.now().toString()
    const expiresAt =
      Date.now() + (urgency === 'emergency' ? 30 * 60 * 1000 : 15 * 60 * 1000) // 30min for emergency, 15min for others
  ...
  private async startRing(pingRequest: PingRequest) {
    ...
    await speechEngine.speak(
      'Your parent is trying to reach you! Tap the notification to respond.',
    )
  ...
  private async promptCheckIn(pingRequest: PingRequest) {
    Alert.alert(
      'Check-in Request',
      pingRequest.message ||
        'Your parent has requested a check-in. How are you doing?',
      [
        {
          text: "I'm OK",
          onPress: () => this.sendCheckInResponse(pingRequest.id, "I'm OK"),
        },
        {
          text: "I'm Safe",
          onPress: () =>
            this.sendCheckInResponse(pingRequest.id, "I'm safe and doing well"),
        },
        {
          text: 'Need Help',
          onPress: () =>
            this.sendCheckInResponse(pingRequest.id, 'I need help', true),
        },
        {
          text: 'Custom Message',
          onPress: () => this.promptCustomCheckIn(pingRequest.id),
        },
      ],
    )
  }
  ...
  private async sendCheckInResponse(
    pingId: string,
    message: string,
    needsHelp = false,
  ) {
  ...
  private async handleEmergencyPing(pingRequest: PingRequest) {
    ...
    Alert.alert(
      '🚨 EMERGENCY PING',
      pingRequest.message ||
        'Your parent has sent an emergency ping. Please respond immediately.',
      [
        {
          text: "I'm Safe",
          onPress: () => this.sendEmergencyResponse(pingRequest.id, 'safe'),
        },
        {
          text: 'I Need Help',
          onPress: () => this.sendEmergencyResponse(pingRequest.id, 'help'),
        },
        {
          text: 'Call Parent',
          onPress: () => this.initiateEmergencyCall(pingRequest.parentId),
        },
      ],
      { cancelable: false },
    )
    ...
    await speechEngine.speak(
      'EMERGENCY: Your parent needs to know you are safe. Please respond right away.',
    )
  }
  ...
  private async sendEmergencyResponse(pingId: string, status: 'safe' | 'help') {
    ...
    if (status === 'help') {
      ...
      Alert.alert(
        'Help Request Sent',
        'Your parent and emergency contacts have been notified.',
      )
    }
  }
  ...
  private async getCurrentLocationSafe(): Promise<{
    latitude: number
    longitude: number
  } | null> {
    try {
    ...
    if (ping) {
      ...
      switch (ping.type) {
        case 'ring':
          Alert.alert(
            'Response Sent',
            'Your parent has been notified that you received their ping.',
          )
          break
        case 'locate':
