...
export class MultiModalRoutePlanner {
  ...
  private static async getPureWalkingRoute(
    ...
    try {
      ...
      const steps: RouteStep[] = route.steps.map((step, index) => ({
        id: `walk-${index}`,
        type: 'walk',
        instruction: this.makeInstructionKidFriendly(step.instruction, 'walking'),
        duration: Math.ceil(step.duration / 60),
        distance: step.distance,
        accessibilityNote: settings.avoidStairs ? 'Flat route selected' : undefined,
      }));
  ...
  private static async getPureBikingRoute(
    ...
    try {
      ...
      const steps: RouteStep[] = route.steps.map((step, index) => ({
        ...
        instruction: this.makeInstructionKidFriendly(step.instruction, 'bicycling'),
        duration: Math.ceil(step.duration / 60),
        distance: step.distance,
    ...
    try {
      ...
      const steps: RouteStep[] = route.steps.map((step, index) => {
        ...
        return {
          ...
          instruction: this.makeInstructionKidFriendly(step.instruction, 'transit'),
          duration: Math.ceil(step.duration / 60),
          distance: step.distance,
          transitLine: step.transitDetails?.line?.name || step.transitDetails?.line,
          transitColor: step.transitDetails?.line?.color || '#000000',
          accessibilityNote: isTransitStep ? 'Stay with adult on transit' : undefined,
        };
      });
  ...
  private static async getCombinedWalkTransitRoute(
    ...
    try {
      ...
      for (const startStation of startStations.slice(0, 3)) { // Limit to top 3
        for (const endStation of endStations.slice(0, 3)) {
          const segments = await this.calculateCombinedRoute(
            from, to, startStation, endStation, settings
          );
      ...
      const totalDistance = bestRoute.reduce((sum, seg) => sum + seg.distance, 0);
   ...
   */
  private static calculateRouteScore(route: RouteOption, settings: MultiModalRouteOptions): number {
    let score = 0;
    ...
    // Age appropriateness
    const ageBonus = this.getAgeAppropriatenessBonus(route.mode, settings.childAge);
    score += ageBonus;
   ...
   */
  private static makeInstructionKidFriendly(instruction: string, mode: TravelMode): string {
    ...
    if (instruction.includes('Continue') || instruction.includes('Head')) {
      const encouragements = ['You\'re doing great! ', 'Keep it up! ', 'Almost there! '];
      friendly = encouragements[Math.floor(Math.random() * encouragements.length)] + friendly;
    }
  ...
  private static addSafetyStops(
    ...
    if (mode === 'walking' || mode === 'bicycling') {
      ...
      enhanced.forEach((step, index) => {
        ...
        if (cumulativeDuration >= safetyInterval && index < enhanced.length - 1) {
          const safetyTips = this.getSafetyTips(mode, settings);
          step.accessibilityNote = (step.accessibilityNote || '') + ' ' + safetyTips;
  ...
  private static addTransitSafetySteps(
    ...
    enhanced.forEach(step => {
      if (step.type === 'transit') {
        ...
        step.accessibilityNote = transitTips[Math.floor(Math.random() * transitTips.length)];
      }
    });
   ...
   */
  private static calculateKidFriendliness(mode: TravelMode | 'combined', settings: MultiModalRouteOptions): number {
   ...
   */
  private static calculateSafetyScore(mode: TravelMode | 'combined', settings: MultiModalRouteOptions): number {
    const baseScores: Record<TravelMode | 'combined', number> = {
      walking: 4,
      bicycling: 3,
      transit: 3,
   ...
   */
  private static getAgeAppropriatenessBonus(mode: TravelMode, childAge: number): number {
   ...
   */
  private static getSafetyTips(mode: TravelMode, settings: MultiModalRouteOptions): string {
    const walkingTips = [
  ...
  private static async calculateCombinedRoute(
    ...
    try {
      ...
      const walkToStation = await fetchRoute(from, [startStation.location.lat, startStation.location.lng], 'walking');
      ...
      if (transitRoute) {
        segments.push({
          ...
          safetyNotes: ['Stay with your adult on transit', 'Hold the handrails'],
          kidFriendlyTips: ['Enjoy the ride!', 'Look out the window!']
        });
      }

      // Walking segment from end station
      const walkFromStation = await fetchRoute([endStation.location.lat, endStation.location.lng], to, 'walking');
      if (walkFromStation) {
        segments.push({
          mode: 'walking',
